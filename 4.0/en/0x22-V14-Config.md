...
| **14.4.5** | Verify that HTTP Strict Transport Security headers are included on all responses and for all sub-domains, such as Strict-Transport-Security: max-age=15724800; includeSubdomains. | ✓ | ✓ | ✓ | 523 |
...
| **14.5.3** | Verify that the Cross-Origin Resource Sharing (CORS) Access-Control-Allow-Origin header uses a strict white-list of trusted domains and sub-domains to match against and does not support the "null" origin. | ✓ | ✓ | ✓ | 346 |

...
| **3.5.4** | [ADDED] Verify expiration of JWTs are checked in the backend service (e.g. exp - expiration time and iat - issued before current time). | ✓ | ✓ | ✓ | 613 | |
| **3.5.5** | [ADDED] Verify that algorithm validation is being done by the backend service for the JWTs used and as such it should not accept "None" value in the algorithm section. | ✓ | ✓ | ✓ | 347 | |
| **3.5.6** | [ADDED] Verify proper validation of the JWT payload claims are done by the backend service (e.g. iss - valid issuer, sub - subject, aud - audience, etc). | ✓ | ✓ | ✓ | 287 | |
| **3.5.7** | [ADDED] Verify that signatures of JWTs are present in the claims to add more protection from tampering. (JWS-JSON Web Signature). | ✓ | ✓ | ✓ | 354 | |
...
| # | Description | L1 | L2 | L3 | CWE | [NIST &sect;](https://pages.nist.gov/800-63-3/sp800-63b.html) |
| :---: | :--- | :---: | :---:| :---: | :---: | :---: |
| **3.6.1** | [MODIFIED] Verify that Relying Parties (RPs) specify the maximum authentication time to Credential Service Providers (CSPs) and that CSPs re-authenticate the user if they haven't used a session within that period. | | | ✓ | 613 | 7.2.1 |

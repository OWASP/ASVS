...
| **13.4.3** | [ADDED] Verify that GraphQL introspection queries are disabled in production environment unless it is a public API where it is being used for integrations to your system. | | ✓ | ✓ |  |
...
| **13.5.4** | [ADDED] Verify that tokens possess at least 64 bits of entropy and are generated using approved cryptographic algorithms if session or channel tokens specific to WebSockets are being used. | ✓ | ✓ | ✓ | 331 |

...
## V13.5 WebSocket Security Requirements

| # | Description | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---:| :---: | :---: |
| **13.5.1** | Verify that only authenticated users can open WebSocket connections. | ✓ | ✓ | ✓ | 306 |
| **13.5.2** | Verify that WebSocket Secure (wss) is used for all WebSocket connections. | ✓ | ✓ | ✓ | 319 |
| **13.5.3** | Verify that, during the initial HTTP WebSocket handshake, the Origin header is checked against a whitelist of authorized origins. | ✓ | ✓ | ✓ | 346 |
| **13.5.4** | Verify that WebSocket messages are properly sanitized and encoded server-side. | ✓ | ✓ | ✓ | 116 |
| **13.5.5** | Verify that rate limiting is in place for WebSocket messages | ✓ | ✓ | ✓ | 770 |
| **13.5.6** | If you are using session or channel tokens in webSockets, verify that the tokens are obtained from a response to secure POST request only, and are not sent by the server through the Websocket connection. | ✓ | ✓ | ✓ | 319 |
| **13.5.7** | If you are using session or channel tokens in webSockets, verify that the tokens possess at least 64 bits of entropy and are generated using approved cryptographic algorithms. | ✓ | ✓ | ✓ | 331 |

...
| **13.2.7** | Verify the requests against a strict json schema, all parameters must have typified: format, extension, range and/or pattern of characters. In the case of arrays, the number of possible elements must also be specified. |  | ✓ | ✓ | 20 |
| **13.2.8** | Reject requests with undefined query parameters or invalidated path parameters to avoid injection problems. | ✓ | ✓ | ✓ | 88 |
| **13.2.9** | Avoid including sensitive data in the url such as password, email, telephone, account number, card number, etc. | ✓ | ✓ | ✓ | 359, 922 |
| **13.2.10** | Validate the headers that are supported and reject the requests with the following headers: X-HTTP-Method-Override, X-Method-Override, X-Content-Type-Options with value different to nosniff, X-Frame- Options with a value different to deny. |  | ✓ | ✓ | 349 |
| **13.2.11** | Avoid including any information that can describe the underlying technology in the error responses, for example adding the stacktrace to the error message. | ✓ | ✓ | ✓ | 211 |
| **13.2.12** | Use standard authorization mechanisms such as: Oauth 2.0 and openId connect avoiding adhoc developments, basic HTTP or HTTP Digest. |  | ✓ | ✓ | 284 |
| **13.2.13** | Use API Key only as an authorization mechanisms. Don't use it for authentication message. | ✓ | ✓ | ✓ | 285 |

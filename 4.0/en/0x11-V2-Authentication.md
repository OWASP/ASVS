...
| **2.1.7** | Verify that passwords submitted during account registration, login, and password change are checked against a set of breached passwords either locally or via API. If the password is breached, the application must require the user to re-choose a non-breached password. | ✓ | ✓ | ✓ | | 5.1.1.2 |
...
| **2.7.5** | Verify that the initial authentication key is generated by a secure random number generator, containing at least 20 bits of entropy (typically a six digital random number is sufficient). |  | ✓ | ✓ | | 5.1.3.2 |
...
| **2.9.4** | Verify that time-based OTPs have a defined lifetime before needing re-seeding or replacement. |  | o | ✓ | | 5.1.5.2 |
| **2.9.6** | Verify that if a time-based OTP token is re-used during the validity period, it is logged and rejected with secure notifications being sent to the holder of the device. |  | ✓ | ✓ | | 5.1.5.2 |
| **2.9.8** | Verify that biometric authenticators are limited to use only as secondary factors in conjunction with either something you have and something you know. |  | o | ✓ | | 5.2.3 |
| **2.9.9** | Verify physical OTP tokens and biometric authenticators can be revoked in case of theft or other loss. Ensure that revocation is immediately effective across logged in sessions, regardless of location. |  | ✓ | ✓ | | 5.2.1 |

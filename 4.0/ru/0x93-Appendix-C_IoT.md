# Приложение C: Требования к исследованию Internet of Things

Эта глава изначально была в основной ветке, но с учетом работы, проделанной командой OWASP IoT, нет смысла поддерживать две разные ветки по этому вопросу. Для версии 4.0 мы переносим это в Приложение и призываем всех, кому это требуется, скорее использовать основной проект [OWASP IoT](https://owasp.org/www-project-internet-of-things/)
...
| **C.13** | Убедитесь, что весь код, включая сторонние бинарные файлы, библиотеки и фреймворки, проверяется на наличие жестко закодированных учетных данных (бэкдоров). | ✓ | ✓ | ✓ | 4.0 |
...
| **C.27** | Убедитесь, что используются только те микроконтроллеры, которые обеспечивают существенную защиту от декапинга (de-capping) и атак по побочным каналам. |  |  | ✓ | 4.0 |

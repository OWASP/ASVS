...
5.0: docker
	docker run --rm --user $(id -u):$(id -g) -v "`pwd`/5.0:/data" -v "`pwd`/docker:/scripts" -e "TARGET=5.0" -e "FORMATS=$(FORMATS)" -e "LANGS=$(LANGS)" ghcr.io/owasp/asvs/documentbuilder:latest
5.0-clean: docker
	docker run --rm --user $(id -u):$(id -g) -v "`pwd`/5.0:/data" -v "`pwd`/docker:/scripts" -e "TARGET=clean" -e "FORMATS=$(FORMATS)" -e "LANGS=$(LANGS)" ghcr.io/owasp/asvs/documentbuilder:latest

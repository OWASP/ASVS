...
Le [NIST SP 800-63](https://pages.nist.gov/800-63-3/) est une norme moderne, fondée sur des preuves, qui est utile à toutes les organisations du monde entier, mais qui est particulièrement pertinente pour les agences américaines et celles qui traitent avec elles.

Bien que les exigences de ce chapitre soient basées sur la deuxième section de la norme (appelée NIST SP 800-63B « Directives relatives à l'identité numérique - Authentification et gestion du cycle de vie »), ce chapitre se concentre sur les menaces courantes et les failles d'authentification fréquemment exploitées. Il ne prétend pas couvrir tous les points de la norme de manière exhaustive. Pour les cas où une conformité totale à la norme NIST SP 800-63 est nécessaire, veuillez vous référer à cette dernière.

De plus, la terminologie du NIST SP 800-63 peut parfois être différente et le chapitre utilise souvent une terminologie plus communément comprise, pour plus de clarté.
...
Cette section détaille les exigences relatives à la documentation d'authentification à conserver pour une application. Elle est essentielle pour mettre en œuvre et évaluer la configuration des contrôles d'authentification pertinents.
...
Cette section contient les exigences générales relatives à la sécurité des mécanismes d'authentification et définit les différentes attentes en matière de niveaux, notamment l'exigence de multifacteur pour L2 et la prise en charge des mécanismes matériels pour L3, tels que les normes fournies par la Fast IDentity Online (FIDO) Alliance.
...
| # | Description | Niveau | #v5.0.be |
| :---: | :--- | :---: | :---: |
| **6.3.1** | Vérifiez que les contrôles visant à empêcher les attaques telles que le bourrage d'informations d'identification et la force brute des mots de passe sont mis en œuvre conformément à la documentation de sécurité de l'application. | 1 | v5.0.be-2.2.1 |
| **6.3.2** | Vérifiez que les comptes d’utilisateur par défaut (par exemple, « root », « admin » ou « sa ») ne sont pas présents dans l’application ou sont désactivés. | 1 | v5.0.be-14.1.10 |
| **6.3.3** | Vérifiez que l’application exige que les utilisateurs utilisent un mécanisme d’authentification multifacteur ou une combinaison de mécanismes d’authentification à facteur unique. | 2 | v5.0.be-2.2.9 |
| **6.3.4** | Vérifiez que, si l’application inclut plusieurs voies d’authentification, il n’existe aucune voie non documentée et que les contrôles de sécurité et la force d’authentification sont appliqués de manière cohérente. | 2 | v5.0.be-2.2.11 |
| **6.3.5** | Vérifiez que les utilisateurs sont informés des tentatives d'authentification suspectes (réussies ou non). Il peut s'agir de tentatives d'authentification provenant d'un emplacement ou d'un client inhabituel, d'une authentification partiellement réussie (un seul facteur parmi plusieurs), d'une tentative d'authentification après une longue période d'inactivité ou d'une authentification réussie après plusieurs tentatives infructueuses. | 3 | v5.0.be-2.2.10 |
| **6.3.6** | Vérifiez que le courrier électronique n’est pas utilisé comme mécanisme d’authentification à facteur unique ou à facteurs multiples. | 3 | v5.0.be-2.2.2 |
| **6.3.7** | Vérifiez que les utilisateurs sont avertis après les mises à jour des détails d'authentification, telles que les réinitialisations d'informations d'identification ou la modification du nom d'utilisateur ou de l'adresse e-mail. | 3 | v5.0.be-2.2.3 |
| **6.3.8** | Vérifiez qu'un mécanisme d'authentification basé sur le matériel est pris en charge, qui offre une résistance à l'usurpation d'identité contre les attaques de phishing (comme WebAuthn) et vérifie l'intention d'authentification en exigeant une action initiée par l'utilisateur (comme une pression sur un bouton sur une clé matérielle FIDO). | 3 | v5.0.be-2.2.4 |
| **6.3.9** | Vérifiez que les utilisateurs valides ne peuvent pas être déduits d'échecs d'authentification, par exemple en se basant sur des messages d'erreur, des codes de réponse HTTP ou des temps de réponse différents. Les fonctionnalités d'inscription et de mot de passe oublié doivent également bénéficier de cette protection. | 3 | v5.0.be-2.2.8 |
...
| # | Description | Niveau | #v5.0.be |
| :---: | :--- | :---: | :---: |
| **6.4.1** | Vérifiez que les mots de passe initiaux ou les codes d'activation générés par le système sont générés de manière aléatoire et sécurisée, qu'ils respectent la politique de mots de passe en vigueur et qu'ils expirent après une courte période ou après leur première utilisation. Ces secrets initiaux ne doivent pas devenir des mots de passe permanents. | 1 | v5.0.be-2.5.8 |
| **6.4.2** | Vérifiez que les indices de mot de passe ou l'authentification basée sur les connaissances (appelées « questions secrètes ») ne sont pas présents. | 1 | v5.0.be-2.5.2 |
| **6.4.3** | Vérifiez qu'un processus sécurisé de réinitialisation d'un mot de passe oublié est mis en œuvre, qui ne contourne aucun mécanisme d'authentification multifacteur activé. | 2 | v5.0.be-2.5.6 |
| **6.4.4** | Vérifiez que si un facteur d'authentification multifacteur est perdu, la preuve de vérification d'identité est effectuée au même niveau que lors de l'inscription. | 2 | v5.0.be-2.5.7 |
| **6.4.5** | Vérifiez que les instructions de renouvellement des mécanismes d'authentification qui expirent sont envoyées avec suffisamment de temps pour être exécutées avant l'expiration de l'ancien mécanisme d'authentification, en configurant des rappels automatiques si nécessaire. | 3 | v5.0.be-2.5.9 |
| **6.4.6** | Vérifiez que les administrateurs peuvent initier la réinitialisation du mot de passe de l'utilisateur, mais que cela ne leur permet pas de modifier ou de choisir son mot de passe. Cela évite qu'ils ne connaissent le mot de passe de l'utilisateur. | 3 | v5.0.be-2.5.10 |
...
Les mécanismes d'authentification hors bande non sécurisés, tels que les e-mails et la VoIP, ne sont pas autorisés. L'authentification par RTC et SMS est actuellement considérée comme des [mécanismes d'authentification « restreints »](https://pages.nist.gov/800-63-FAQ/#q-b01) par le NIST et devrait être abandonnée au profit des notifications push ou similaires. La norme NIST SP 800-63B [&sect; 5.1.3.3](https://pages.nist.gov/800-63-3/sp800-63b.html#-5133-authentication-using-the-public-switched-telephone-network) fournit des indications sur la nécessité impérative de prendre en charge l'authentification hors bande par téléphone ou SMS.
| # | Description | Niveau | #v5.0.be |
| :---: | :--- | :---: | :---: |
| **6.6.1** | Vérifier que les mécanismes d'authentification utilisant le réseau téléphonique public commuté (RTPC) pour la transmission de mots de passe à usage unique (OTP) par téléphone ou SMS ne sont proposés que lorsque d'autres méthodes plus robustes (telles que les notifications push) sont également proposées et que le service informe les utilisateurs des risques de sécurité qu'elles représentent. Pour les applications de niveau 3, le téléphone et les SMS ne doivent pas être disponibles. | 2 | v5.0.be-2.7.1 |
...
Les mécanismes d'authentification cryptographique incluent les cartes à puce ou les clés FIDO. L'utilisateur doit connecter ou appairer le dispositif cryptographique à l'ordinateur pour finaliser l'authentification. Le serveur d'authentification envoie un nonce de défi au dispositif ou au logiciel cryptographique, qui calcule une réponse à partir d'une clé cryptographique stockée de manière sécurisée. Les exigences de cette section fournissent des conseils spécifiques à la mise en œuvre de ces mécanismes, les recommandations relatives aux algorithmes cryptographiques étant abordées dans le chapitre « Cryptographie ».

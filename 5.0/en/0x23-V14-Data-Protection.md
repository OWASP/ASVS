...
| :---: | :--- | :---: | :---: |
| **14.1.1** | Verify that all sensitive data created and processed by the application has been identified and classified into protection levels. This includes data that is only encoded and therefore easily decoded, such as Base64 strings or the plaintext payload inside a JWT. | 2 | v5.0.be-1.8.1 |
...
| **14.2.5** | Verify that caching mechanisms only cache responses with static, cache-safe content types (e.g., images, CSS, JavaScript, fonts). Responses that contain dynamic or sensitive data should not be cached. When a non-existent resource is requested, the server must return 404 or 302 rather than another valid file to prevent Web Cache Deception attacks. | 3 | v5.0.be-8.1.7 |
| **14.2.6** | Verify that the application only returns the minimum required sensitive data for the application's functionality. If the complete data is required, it should be masked in the user interface unless the user specifically views it. | 3 | v5.0.be-8.1.10 |
| **14.2.7** | Verify that sensitive or confidential information is subject to data retention classification, ensuring that outdated or unnecessary data is deleted automatically, on a defined schedule, or as the situation requires. | 3 | v5.0.be-8.3.8 |

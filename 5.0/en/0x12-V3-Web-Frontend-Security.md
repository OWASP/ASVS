...
| # | Description | Level |
| :---: | :--- | :---: |
| **3.4.1** | Verify that a Strict-Transport-Security header field is included on all responses to enforce an HTTP Strict Transport Security (HSTS) policy. The 'max-age' parameter must be set to at least 31536000 (one year). For L2 and up, the policy must apply to all subdomains as well. | 1 |

...
When considering using JSON schema validation, which is best practice for RESTful web services, consider using these additional data validation strategies in combination with JSON schema validation:
...
The JSON schema validation standard is now considered formalized. It is important to ensure that the implementation you are using is up to date with the schema version.

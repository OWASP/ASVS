...
| **13.4.6** | Verify that the integrity and security posture of deployed environments are maintained over time, ensuring configurations do not drift into an insecure state. This can be achieved by using short-lived environments frequently redeployed from a secure baseline ("known good state") or by employing drift prevention mechanisms for long-lived environments. | 3 | v5.0.be-14.1.5 |
...
| **13.5.2** | Verify that directory browsing is disabled in the web server configuration unless deliberately desired. | 2 | v5.0.be-14.3.4 |

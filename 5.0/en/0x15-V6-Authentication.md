...
| **6.3.3** | Verify that either a multi-factor authentication mechanism or a combination of single-factor authentication mechanisms (such as cryptographic authentication in conjunction with a password), must be used in order to access the application. Relaxing this requirement requires a fully documented rationale and a comprehensive set of mitigating controls. | 2 | v5.0.be-2.2.9 |
...
| **6.3.8** | Verify that a hardware-based authentication mechanism (such as device-bound passkeys, eIDAS LoA3 enforced authenticators pt, or authenticators with AAL3 assurance, or the equivalent thereof) are required and provides compromise- and impersonation resistance against phishing attacks while verifying the intent to authenticate by requiring a user-initiated action (such as a button press on a FIDO hardware key or a mobile phone). | 3 | v5.0.be-2.2.4 |
| **6.3.9** | Verify that valid users cannot be deduced from failed authentication challenges, such as by basing on error messages, HTTP response codes, or different response times. Registration and forgot password functionality must also have this protection. | 3 | v5.0.be-2.2.8 |

...
| **1.1.3** | Verify that input is transformed into a single, standard, and simplified (canonical) form beyond basic decoding or unescaping. This includes handling variations in case, whitespace, equivalent representations, and other normalization steps to ensure consistent processing and prevent bypasses based on subtle differences in input. | 2 | v5.0.be-5.6.3 |
| **1.1.4** | Clearly define and architecturally separate the processes of validation (determining if input meets expected criteria) and sanitization (modifying input to make it safe). Emphasize that validation should ideally occur before sanitization to avoid sanitizing invalid data or masking underlying issues. | 2 | v5.0.be-5.6.4 |
| **1.1.5** | Verify that validation and sanitization rules are context-aware. The expected format and allowed characters for a username might differ significantly from those for a product description. The sanitization techniques applied should also be appropriate for the specific data type and its intended use. | 2 | v5.0.be-5.6.5 |
| **1.1.6** | For applications processing structured data formats, verify that parsing and schema validation occur early in the process to ensure the data adheres to the expected structure before further processing, validation, or sanitization of individual data points within the structure. | 2 | v5.0.be-5.6.6 |
| **1.1.7** | Reinforce the principle that data should be stored in its original, canonicalized form (after initial decoding/unescaping and normalization) without output encoding. Output encoding should be applied as the absolute final step, just before the data is rendered or transmitted to the consuming interpreter, based on the specific output context. | 2 | v5.0.be-5.6.7 |
| **1.1.8** | Verify that the application provides appropriate and secure feedback to the user or system regarding validation failures. Error messages should be informative enough for legitimate users to correct their input but should not reveal sensitive information to potential attackers. Additionally, consider how sanitization actions are logged or communicated, if at all. | 2 | v5.0.be-5.6.8 |
| **1.1.9** | Consider the architectural design for how validation and sanitization rules are defined, managed, and updated. The architecture should allow for easy addition of new rules and modification of existing ones without requiring significant code changes, promoting maintainability and adaptability to evolving threats. | 2 | v5.0.be-5.6.9 |
